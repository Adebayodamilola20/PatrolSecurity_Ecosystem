        ...
        builder: (ctx, setModalState) => AlertDialog(
          ...
          content: SingleChildScrollView(
            child: Column(
              ...
              children: [
                Text(order.instructions, style: const TextStyle(color: AppTheme.textSecondary, height: 1.4)),
                ...
                OutlinedButton.icon(
                  ...
                  label: Text(photo == null ? 'Capture proof photo' : 'Retake proof photo'),
                ),
                if (photo != null) ...[
                  const SizedBox(height: 12),
                  ClipRRect(
                    borderRadius: BorderRadius.circular(12),
                    child: Image.file(File(photo!.path), height: 140, width: double.infinity, fit: BoxFit.cover),
                  ),
                ],
              ],
            ),
          ...
          actions: [
            ...
            FilledButton(
              onPressed: duty.submitting || photo == null
                  ? null
                  : () async {
                      final ok = await duty.complete(orderId: order.id, photo: File(photo!.path), note: notesCtrl.text.trim());
        ...
        builder: (ctx, setModalState) => AlertDialog(
          ...
          content: SingleChildScrollView(
            child: Column(
              mainAxisSize: MainAxisSize.min,
              children: [
                TextField(controller: summaryCtrl, maxLines: 3, decoration: const InputDecoration(labelText: 'Shift summary *')),
                const SizedBox(height: 12),
                TextField(controller: issuesCtrl, maxLines: 3, decoration: const InputDecoration(labelText: 'Open issues')),
                const SizedBox(height: 12),
                TextField(controller: equipmentCtrl, maxLines: 2, decoration: const InputDecoration(labelText: 'Equipment status')),
                const SizedBox(height: 12),
                OutlinedButton.icon(
                  onPressed: () async {
                    final picked = await _picker.pickImage(source: ImageSource.camera, imageQuality: 80);
                  ...
                  icon: const Icon(Icons.camera_alt_outlined),
                  label: Text(photo == null ? 'Add handover photo' : 'Retake handover photo'),
                ),
              ],
            ),
          ),
      ...
      appBar: AppBar(
        ...
        actions: [
          TextButton.icon(
            ...
            label: const Text('Handover', style: TextStyle(color: AppTheme.primary)),
          ),
        ],
      ),
        ...
        child: ListView(
          ...
          children: [
            if (duty.pendingHandovers.isNotEmpty) ...[
              const Text('Pending Handovers', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w700, color: AppTheme.text)),
              const SizedBox(height: 12),
              ...duty.pendingHandovers.map((handover) => Card(
                    child: Padding(
                      padding: const EdgeInsets.all(16),
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          ...
                          Align(
                            ...
                            child: FilledButton(
                              onPressed: duty.submitting ? null : () => duty.acceptHandover(handover.id),
            ...
            if (duty.orders.isEmpty)
              const Card(
                child: Padding(
                  ...
                  child: Text('No active post orders right now.', style: TextStyle(color: AppTheme.textSecondary)),
                ),
              ),
